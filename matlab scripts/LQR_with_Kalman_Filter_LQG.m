...
% Ki(3,1) = -31.6228 * 0.6;
Ki(1,3) = -31.6228/3;  % or even -1.0 to start
...
% Kalman Filter Design for Linearized Quadcopter System

% A, B matrices already defined from linearized model
% Let's define a C matrix for measuring position and orientation only
% States: [x y z phi theta psi u v w p q r]
% Measured outputs: [x y z phi theta psi] → 6 outputs
% 
% C = [eye(6), zeros(6,6)];   % C is 6x12
% D = zeros(6,4);             % D is 6x4 (no direct feedthrough)
% 
% % Define process and measurement noise covariance matrices
% Q = 0.01 * eye(12);   % Process noise covariance (tunable)
% R = 0.05 * eye(6);    % Measurement noise covariance (tunable)
% 
% % Solve the continuous-time Algebraic Riccati Equation
% P = care(A', C', Q, R);  % CARE: P*A' + A*P - P*C'*R^-1*C*P + Q = 0
% 
% % Compute Kalman gain
% K_kalman = P * C' / R;
% 
% % Display or save the Kalman gain
% disp('✅ Kalman Gain (K):');
% disp(K_kalman);
% 
% % Optional: save for Simulink use
% save('KalmanGain.mat', 'K_kalman');
% 
% ShapeU = eye(4);


% System Matrices (from your quadrotor linearized model)

C = eye(6, 12);  % Assume x, y, z, φ, θ, ψ are measured
D = zeros(6, 4);

% Define noise covariances
Q = 0.1 * eye(12);  % Process noise
R = 0.2 * eye(6);   % Measurement noise
...
A_obs = A - K * C;
B_obs = [B, K];                  % [u; y] as input
C_obs = eye(12);                % Output is the full state estimate
D_obs = zeros(12, 10);          % 4 inputs + 6 outputs

% Use these matrices in a State-Space block in Simulink
